...
cp -fr jflteatt_active/2nd-init-files/* $OUT/2nd-init-files/
cp -fr jf-common/res/* $OUT/install-files/etc/safestrap/res/
cp -fr jflteatt_active/sbin/* $OUT/recovery/root/sbin/
...
cp -fr jflteatt_active/hijack $OUT/install-files/etc/init.qcom.modem_links.sh
