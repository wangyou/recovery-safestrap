...
cp -fr jfltevzw/2nd-init-files/* $OUT/2nd-init-files/
cp -fr jfltevzw/res/* $OUT/install-files/etc/safestrap/res/
cp -fr jfltevzw/sbin/* $OUT/recovery/root/sbin/
...
cp -fr msm8960-common/hijack $OUT/install-files/etc/init.qcom.modem_links.sh
