...
if [ ! -f "$DESTMOUNT/bin/bootmodem.bin" ]; then
	...
	$BBX chown 0.2000 $DESTMOUNT/bin/bootmodem.bin >> $LOGFILE
	$BBX chmod 755 $DESTMOUNT/bin/bootmodem.bin >> $LOGFILE
fi
$BBX rm $DESTMOUNT/bin/bootmodem >> $LOGFILE
$BBX cp -f $INSTALLPATH/install-files/bin/bootmodem $DESTMOUNT/bin/bootmodem >> $LOGFILE
$BBX chown 0.2000 $DESTMOUNT/bin/bootmodem >> $LOGFILE
$BBX chmod 755 $DESTMOUNT/bin/bootmodem >> $LOGFILE
