...
bool TWPartition::Process_Flags(string Flags, bool Display_Error) {
	...
	while (index < flags_len) {
		...
		if (strcmp(ptr, "removable") == 0) {
			...
			Fstab_Options = ptr;
			if (Fstab_Options.substr(Fstab_Options.size() - 1, 1) == "\"") {
				Fstab_Options.resize(Fstab_Options.size() - 1);
			}
			Process_FS_Flags(Fstab_Options, Fstab_Flags);
			LOGINFO("Found Flags=0x%8x, Options=%s\n", Fstab_Flags, Fstab_Options.c_str());
		} else {
			if (Display_Error)
...
bool TWPartition::Mount(bool Display_Error) {
	...
	} else if (!exfat_mounted && mount(Actual_Block_Device.c_str(), Mount_Point.c_str(), Current_File_System.c_str(), Fstab_Flags, Fstab_Options.c_str()) != 0) {
#ifdef TW_NO_EXFAT_FUSE
